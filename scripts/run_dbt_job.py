...
def main():
    ...
    raw_dir = settings.runtime.temp_raw_dir
    ...
    result = subprocess.run(
        ["dbt", "build"],
        cwd=settings.runtime.dbt_project_dir,
        capture_output=True,
        text=True,
    )
    ...
    con = duckdb.connect(str(settings.runtime.dbt_database_path))
    gold_dir = settings.runtime.temp_gold_dir
    gold_dir.mkdir(parents=True, exist_ok=True)
    con.execute(f"EXPORT DATABASE '{gold_dir}' (FORMAT PARQUET)")
